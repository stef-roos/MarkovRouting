...
public class FailureAlpha3Beta2UpperMult extends KadTypeUpper2{
	double fprob;

public FailureAlpha3Beta2UpperMult(int b, int[] k, double[][] l,
...
public FailureAlpha3Beta2UpperMult(int b, int[] k, int l, double fprob) {
	super(b, 3, 2, k, l);
	this.fprob = fprob;
}
...
protected void processCDFsT2(int n, double[][] t2, int[] old, int oldindex,
	...
	if (this.ltype == LType.SIMPLE){
	for (int i1 = 0; i1 <= old[0]+1; i1++){
		returned[0][0] = i1<=old[0]?i1:old[2];
		for (int i2 = i1; i2 <= Math.max(old[0],i1); i2++){
			returned[0][1] = i1<=old[0]?i2:old[2];
			   double p1 = i1<=old[0]?this.getProb(returned[0], old[0],0)*(1-this.fprob):this.fprob;
			for (int j1 = 0; j1 <= old[1]+1; j1++){
				returned[1][0] = j1<=old[1]?j1:old[2];;
				for (int j2 = j1; j2 <= Math.max(old[1],j1); j2++){
					returned[1][1] = j1<=old[1]?j2:old[2];
					double p2 = j1<=old[1]?this.getProb(returned[1], old[1],0)*(1-this.fprob):this.fprob;
					for (int k1 = 0; k1 <=old[2]+1; k1++){
						returned[2][0] = k1<=old[2]?k1:old[2];;
						for (int k2 = k1; k2 <= Math.max(old[2],k1); k2++){
							returned[2][1] = k1<=old[2]?k2:old[2];
							double p3 = k1<=old[2]?this.getProb(returned[2], old[2],0)*(1-this.fprob):this.fprob;
	...
	if (this.ltype == LType.ALL){
		for (int a1 = 1; a1 <= old[0]; a1++){
			...
			for (int i1 = 0; i1 <= old[0]+1; i1++){
				returned[0][0] = i1<= old[0]+1?i1:old[2];
				for (int i2 = i1; i2 <= Math.max(old[1],i1); i2++){
					returned[0][1] = i1<=old[0]?i2:old[2];
					 double p1 = i1<=old[0]?
							   ...
							   for (int j1 = 0; j1 <= old[1]+1; j1++){
								...
								for (int a3 = 1; a3 <= old[2]; a3++){
									...
									for (int k1 = 0; k1 <=old[2]+1; k1++){
										returned[2][0] = k1<=old[2]?k1:old[2];;
										for (int k2 = k1; k2 <= Math.max(old[2],k1); k2++){
											returned[2][1] = k1<=old[2]+1?k2:old[2];
										double p3 = k1<=old[2]?this.getProb(returned[2], old[2],a3-1)
