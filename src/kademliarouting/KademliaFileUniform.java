...
public class KademliaFileUniform {
	 ...
	 * @param nodes
	 */
	public KademliaFileUniform(int bits, int[] k, int nodes){
		this.bits = bits;
		this.k = k;
		this.n = nodes;
		this.setNF();
	 ...
	 * @param nodes
	 */
	public KademliaFileUniform(int bits, int nodes){
		this.bits = bits;
	 ...
	 * @param nodes
	 */
	public KademliaFileUniform(int bits, int nodes, int k1){
		this.bits = bits;
		this.k = new int[bits+1];
	...
	private void setNF(){
		...
		for (int j = 0; j < notfound.length; j++){
		 for (int i = 0; i < notfound[j].length; i++){
			...
			for (int x = j+2; x < this.n-alpha; x++){
				notfound[j][i] = Math.min(notfound[j][i] + getRT(x,j)*Calc.binomDist(this.n-alpha-1, x, p),1);
