...
public class Alpha4Beta3Upper2 extends KadTypeUpper2 {

	public Alpha4Beta3Upper2(int b, int[] k, double[][] l,
			LType ltype) {
		super(b, 4, 3, k, l, ltype);
	}
	
	public Alpha4Beta3Upper2(int b, int[] k, int l) {
		super(b, 4, 3, k, l);
	}
			...
			for (int a1 = 1; a1 <= old[0]; a1++) {
				...
				for (int i1 = 0; i1 < this.cdfs[old[0]].length-a1+1; i1++) {
					...
					for (int i2 = i1; i2 < this.cdfs[old[0]].length-a1+1; i2++) {
						returned[0][1] = i2;
						for (int i3 = i2; i3 < this.cdfs[old[0]].length-a1+1; i3++) {
							returned[0][2] = i3;
							...
							for (int a2 = 1; a2 <= old[1]; a2++) {
								...
								for (int j1 = 0; j1 < this.cdfs[old[1]].length-a2+1; j1++) {
									returned[1][0] = j1;
									for (int j2 = j1; j2 < this.cdfs[old[0]].length-a2+1; j2++) {
										returned[1][1] = j2;
										for (int j3 = j2; j3 < this.cdfs[old[0]].length-a2+1; j3++) {
											returned[1][2] = j3;
											double p2 = this.getProb(
													returned[1], old[1],a2-1)
													* l[old[1]][a2];
											for (int a3 = 1; a3 <= old[2]; a3++) {
												...
												for (int k1 = 0; k1 < this.cdfs[old[2]].length-a3+1; k1++) {
													...
													for (int k2 = k1; k2 < this.cdfs[old[2]].length-a3+1; k2++) {
														returned[2][1] = k2;
														for (int k3 = k2; k3 < this.cdfs[old[2]].length-a3+1; k3++) {
															returned[2][2] = k3;
															double p3 = this
																	.getProb(
																			returned[2],
